      ...
      <div class="login-card">
        <div class="logo">
          <svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor">
            <path d="M12 .3a12 12 0 00-3.8 23.4c.6.1.8-.3.8-.6v-2c-3.3.7-4-1.6-4-1.6-.6-1.4-1.4-1.8-1.4-1.8-1.1-.7.1-.7.1-.7 1.2.1 1.9 1.2 1.9 1.2 1.1 1.9 2.9 1.3 3.6 1 .1-.8.4-1.3.8-1.6-2.7-.3-5.5-1.3-5.5-5.9 0-1.3.5-2.4 1.2-3.2-.1-.3-.5-1.5.1-3.2 0 0 1-.3 3.3 1.2a11.5 11.5 0 016 0c2.3-1.5 3.3-1.2 3.3-1.2.6 1.7.2 2.9.1 3.2.8.8 1.2 1.9 1.2 3.2 0 4.6-2.8 5.6-5.5 5.9.4.4.8 1.1.8 2.2v3.2c0 .3.2.7.8.6A12 12 0 0012 .3"></path>
          </svg>
        </div>
        <h1>登录后台</h1>
        ...
        <form id="loginForm" autocomplete="off" style="text-align:left">
          ...
          <div style="position:relative;margin-bottom:14px">
            <input type="password" id="tokenInput" placeholder="ghp_xxx 或 github_pat_xxx"
              style="width:100%;height:42px;border:1px solid var(--border-strong);border-radius:6px;padding:0 40px 0 14px;font-size:14px;font-family:'SF Mono',Monaco,Consolas,monospace"
              required>
            <button type="button" id="togglePwd" tabindex="-1"
              style="position:absolute;right:8px;top:50%;transform:translateY(-50%);width:28px;height:28px;color:var(--text-tertiary)" title="显示/隐藏">
...
function renderHeaderUser() {
  ...
  $('#navActions').innerHTML = `
    ...
    <div class="nav-avatar" style="background-image:url(${escapeHtml(user && user.avatar_url || '')})" title="${escapeHtml(user ? user.login : '')}"></div>
...
async function renderDashboard() {
  ...
  try {
    // 优先从 GitHub API 获取（更新及时），失败则 fallback 到静态文件
  ...
  function refresh(query = '') {
    ...
    if (!filtered.length) {
      list.innerHTML = '<div class="empty">没有文章。点击右上角「+ 新建」开始第一篇。</div>';
      return;
    }
    list.innerHTML = `
      ...
      ${filtered.map(p => `
        <div class="admin-row" data-slug="${escapeHtml(p.slug)}">
          ...
          <div class="meta">${(p.tags || []).length}</div>
          <div class="actions">
            <a href="../post.html?slug=${encodeURIComponent(p.slug)}" target="_blank">查看</a>
